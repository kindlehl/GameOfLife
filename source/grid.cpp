...
void Grid::tick(){
	...
	//load all neighbor counts
	for(int i = 0; i < creatures.size(); i++){
		cellCounts.push_back(neighborCount(i));
		if(creatures[i].alive() && (cellCounts[i] < 2 || cellCounts[i] > 3)){
			killList.push_back(i);
		}
		else if(creatures[i].dead() && cellCounts[i] == 3){
...
sf::RectangleShape Grid::getRect(int screenWidth, int screenHeight, int index)const{
	...
	int h = screenHeight/rows;			//components of rect
	int x = w*(index%cols);
	int y =	h*(index/cols);	
...
void Grid::toggleByClick(int screenWidth, int screenHeight, int x, int y){
	...
	std::cout << "Index in grid: " << rowNumber*rows + colNumber << std::endl;
}
